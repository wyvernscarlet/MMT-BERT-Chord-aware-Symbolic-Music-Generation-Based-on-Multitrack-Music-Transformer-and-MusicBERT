...
class OctupleEncoder(TransformerSentenceEncoder):
    def __init__(self, *args, **kwargs) -> None:
        ...
        if not disable_cp:
            ...
            self.upsampling = nn.Sequential(
                nn.Linear(embedding_dim, embedding_dim * 6))
    ...
    def forward(
        ...
        if self.embed_positions is not None:
            temp = self.embed_positions(tokens[:, ::ratio], positions=positions)
            x = x + \
                temp[...,0]
        ...
        if padding_mask is not None:
            b = 1 - padding_mask.unsqueeze(-1).type_as(x)
            b = torch.squeeze(b,3)
            x = x * (1 - b)
        x = x.transpose(0, 1)
        inner_states = []
        ...
        for layer in self.layers:
            if padding_mask is not None:
                x, _ = layer(x, self_attn_padding_mask=padding_mask[...,0])
            if not last_state_only:
                inner_states.append(x)
        if not disable_cp:
            x = x.transpose(0, 1)
...
class MusicBERTModel(RobertaModel):
    @classmethod
    def build_model(cls, args, dict):
        base_architecture(args)
        if not hasattr(args, "max_positions"):
            args.max_positions = args.tokens_per_sample
        encoder = MusicBERTEncoder(args, dict)
        return encoder
...
def base_architecture(args):
    ...
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 6)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 6)
