...
class MusicXTransformer(nn.Module):
    ...
    def derive(self, seq_in, seq_len, **kwargs):
        return self.decoder.derive(seq_in, seq_len, **kwargs)


class TransformerBlock(nn.Module):
    def __init__(self, input_dim, model_dim=128, num_heads=8, ff_dim=64, dropout=0.2):
        ...
        self.query = nn.Linear(input_dim, model_dim)
        self.key = nn.Linear(input_dim, model_dim)
        self.value = nn.Linear(input_dim, model_dim)
        self.dim_transform = nn.Linear(input_dim, model_dim)
        self.dim_transform_back = nn.Linear(model_dim, input_dim)
    ...
    def forward(self, x):
        ...
        x = self.dim_transform_back(x)

        return x


class Discriminator(nn.Module):
    def __init__(self, num_heads=8, ff_dim=32, num_transformer_blocks=1, mlp_units=256, dropout=0.2,
                 ignore_index=-100, pad_value=0):
        super(Discriminator, self).__init__()
        self.dis_args = argparse.Namespace()
        self.dis_args.tokens_per_sample = 8192
        self.filename = "dict.txt"
        self.dict = Dictionary.load(self.filename)
        self.net = musicbert.MusicBERTModel.build_model(self.dis_args, self.dict)
        self.linear1 = nn.Linear(1236, 6)
        # self.linear2 = nn.Linear(6,1)
        # self.sigmoid = nn.Sigmoid()
        # self.global_avg_pooling = nn.AdaptiveAvgPool1d(1)
        self.prerelu = nn.PReLU()
        self.gelu = nn.GELU()
